...
export class World {
  ...
  _placeFeatures(){
    ...
    // Sprint 12: home chest interactable. The home map declares a
    // `homeChest: {x,y,w,h,name}` in its def; we materialise it on the
    // world as a single non-loot interactable. State is global, not per-map,
    // so no `idx` / `opened` flag — the chest is always available in home.
    if(this.def.homeChest){
      const hc = this.def.homeChest;
      if(this.map[hc.y]&&this.map[hc.y][hc.x]!==undefined) this.map[hc.y][hc.x]=T.FLOOR;
      // Decor generated by the interior path can land on top of the home
      // chest tile. Filter it out so the chest is interactable from all
      // four sides, not hidden behind a barrel.
      this.decor = (this.decor||[]).filter(d => !(d.x === hc.x && d.y === hc.y));
  ...
  reservedZones(def){
    ...
    if(def.homeChest) out.push({x:def.homeChest.x*t+t/2, y:def.homeChest.y*t+t/2});
    return out;
  }
  ...
  draw(ctx, cam){
    ...
    // Sprint 12: render the home chest (if this map has one). Always drawn
    // open and slightly distinct from dungeon chests so the player can
    // tell at a glance that this is *their* chest, not a one-time loot box.
    if(this.homeChest){
      const hc = this.homeChest;
      const sx = hc.x*TILE-cam.x, sy = hc.y*TILE-cam.y;
      // Same shape as a regular chest, but the lid is up and the body has a
      // warm wood tone (slightly redder brown) to differentiate.
      ctx.fillStyle = '#9a5a2a'; ctx.fillRect(sx+6, sy+10, 20, 16);
      ctx.fillStyle = '#6a3a1a'; ctx.fillRect(sx+6, sy+6,  20, 6);   // open lid
      ctx.fillStyle = '#ffcf4d'; ctx.fillRect(sx+14, sy+16, 4, 4);   // gold clasp
    }
